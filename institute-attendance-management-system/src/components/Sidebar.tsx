...
const navItems: NavItem[] = [
  { id: 'asistencias', label: 'Asistencia', icon: <ClipboardList className="w-5 h-5" />, roles: ['admin', 'docente'] },
...
const Sidebar: React.FC = () => {
  const { currentUser, logout, activeSection, setActiveSection, sidebarOpen, setSidebarOpen } = useApp();
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        {sidebarOpen ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[15px] font-bold leading-tight tracking-tight text-foreground">Asistencias</p>
              <p className="text-xs text-muted-foreground font-medium">Trabajo diario</p>
            </div>
          </div>
        ) : (
        ...
        <button
          ...
          title={sidebarOpen ? 'Contraer menu' : 'Expandir menu'}
        >
