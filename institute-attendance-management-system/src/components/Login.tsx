...
const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-900 grid lg:grid-cols-[1.08fr_.92fr]">
      <section className="relative min-h-[46vh] lg:min-h-screen overflow-hidden bg-slate-950 px-6 py-8 flex items-end">
        ...
        <div className="relative max-w-2xl">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-sm text-white/85 mb-5">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 max-w-lg">
            {['Admin', 'Docentes', 'Alumnos'].map(item => (
              <div key={item} className="rounded-lg border border-white/10 bg-white/10 px-3 py-3">
      ...
      <section className="px-5 py-8 sm:px-10 lg:px-12 flex items-center">
        <div className="w-full max-w-lg mx-auto">
          ...
          <div className="my-6 flex items-center gap-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Acceso local</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-sm font-semibold text-slate-800 mb-3">Entrada local manual</p>
            <div className="grid sm:grid-cols-[1fr_120px] gap-2">
              <input
                type="email"
                value={email}
                onChange={e => setEmail(e.target.value)}
                className="h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:ring-2 focus:ring-amber-400"
                placeholder="correo"
              />
              <input
                type="password"
            ...
            <button className="mt-3 w-full h-10 rounded-lg border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              Ingresar
            </button>
          </form>
        </div>
